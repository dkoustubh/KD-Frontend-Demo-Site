...
import { DirectoryScreen } from '../screens/team/DirectoryScreen';
import { SignOut, House, Users, Gear, Database, ChartLineUp, CaretRight } from 'phosphor-react-native';
...
const CustomDrawerContent = (props: any) => {
  ...
  const menuItems = [
    { label: 'System Overview', icon: House, route: 'Dashboard' },
    { label: 'Directory', icon: Users, route: 'Directory' },
    { label: 'System Logs', icon: Database, route: 'Logs' },
    { label: 'Activity Map', icon: ChartLineUp, route: 'Activity' },
  ...
  return (
    <View style={styles.drawerContainer}>
              ...
              <Animated.View key={index} entering={FadeInDown.delay(index * 50).springify()}>
                ...
                >
                  <View style={[styles.iconBox, isActive && styles.activeIconBox]}>
                    <Icon size={20} color={isActive ? '#3B82F6' : colors.textSecondary} weight={isActive ? "fill" : "duotone"} />
                  </View>
                  <Text style={[styles.menuText, isActive && styles.activeMenuText]}>{item.label}</Text>
                  {isActive && <Animated.View entering={SlideInLeft} style={[styles.activeIndicator, { backgroundColor: '#3B82F6' }]} />}
      ...
      <View style={styles.footer}>
        ...
        <Text style={styles.versionText}>System v1.0.2 Admin</Text>
      </View>
    </View>
  );
};
...
export const AdminNavigator = () => {
  return (
    ...
    >
      ...
      <Drawer.Screen name="Logs" component={AdminDashboardScreen} />
      <Drawer.Screen name="Activity" component={AdminDashboardScreen} />
      <Drawer.Screen name="Settings" component={AdminDashboardScreen} />
    </Drawer.Navigator>
  );
};
...
const styles = StyleSheet.create({
  ...
  avatarText: { color: '#1E293B', fontSize: 22, fontWeight: '800' },
  ...
  activeMenuItem: { backgroundColor: 'rgba(59, 130, 246, 0.1)' },
  ...
  activeIconBox: { backgroundColor: '#FFF', borderColor: 'rgba(59, 130, 246, 0.2)' },
  menuText: { fontSize: 15, color: colors.textSecondary, marginLeft: 12, fontWeight: '600', flex: 1 },
  activeMenuText: { color: '#3B82F6', fontWeight: '800' },
